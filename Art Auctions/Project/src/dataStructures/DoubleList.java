...
public class DoubleList<E> implements List<E> {
    ...
    protected DoubleListNode<E> getNode( int position ) {
        ...
        if ( position <= ( currentSize - 1 ) / 2 ) {
            ...
            node = tail;
            for ( int i = currentSize - 1; i > position; i-- ) node = node.getPrevious();
        }
        return node;
    }
     ...
     * @Pre: position ranges from 1 to currentSize-1.
     */
     ...
     * @Pre: the node is neither the head nor the tail of the list.
     */
    ...
    public E remove( int position ) throws InvalidPositionException {
        ...
        if ( position == 0 ) this.removeFirstNode();
        else if ( position == currentSize - 1 ) this.removeLastNode();
        else this.removeMiddleNode(node);
    ...
    protected DoubleListNode<E> findNode( E element ) {
        DoubleListNode<E> node = head;
        while (node != null && !node.getElement().equals(element)) node = node.getNext();
        return node;
    }
    ...
    public boolean remove( E element ) {
        ...
        if ( node == null ) return false;
            ...
            if ( node == head ) this.removeFirstNode();
            else if ( node == tail ) this.removeLastNode();
            else this.removeMiddleNode(node);
            return true;
        }
    }
